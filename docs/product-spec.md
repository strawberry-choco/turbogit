...
- **Git engine — in-process libgit2 primary, CLI retained for sync/fallback** (revised 2026-08-26; supersedes the 2026-07-31 CLI-first + `gix` plan):
  - **Primary executor:** [`git2`](https://docs.rs/git2) (libgit2 bindings) behind the typed `GitExecutor` seam (`turbogit-engine-api`, adapters in `turbogit-engine`). Covers local mutations and reads — commit, branch create/delete/rename/checkout, tag create/list, stash push/pop/drop/apply, stage/unstage via index writes, forward hunk-level index apply, status/log/diff plumbing.
