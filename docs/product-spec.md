...
>
> **Scope.** Desktop-class Git client UX modeled on IntelliJ IDEA 2026.x. Built as a set of cooperating modules over a real Git engine (libgit2 / git CLI), not a toy.
>
...
**Non-goals (v1).**
- Replacing the Git engine internals — we wrap git CLI / libgit2.
...
flowchart LR
  subgraph Engine["Git Engine Layer"]
    GE["Git Executor<br/>(CLI / libgit2 wrapper)"]
...
|---|---|---|
| **0. Engine & Core** | Git Executor (CLI/libgit2), VCS Manager, single Root model, directory mappings, status. | A project shows real Git status for one root. |
