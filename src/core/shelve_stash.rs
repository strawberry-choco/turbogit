...
//!
//! The stash functions are thin pass-throughs to [`VcsManager`]. The shelf is a
...
pub fn stash_apply_index(vcs: &VcsManager, root: &Path, index: usize) -> TgResult<()> {
    vcs.stash_apply(root, index)
}

/// Apply the stash at `index` and drop it.
pub fn stash_pop_index(vcs: &VcsManager, root: &Path, index: usize) -> TgResult<()> {
    vcs.stash_pop(root, index)
}

/// Drop the stash at `index`.
pub fn stash_drop_index(vcs: &VcsManager, root: &Path, index: usize) -> TgResult<()> {
    vcs.stash_drop(root, index)
}

/// Create a branch from a stash entry (`git stash branch`).
pub fn stash_branch(vcs: &VcsManager, root: &Path, index: usize, branch: &str) -> TgResult<()> {
    vcs.branch_create(root, branch, true, Some(&format!("stash@{{{}}}", index)))
}

/// List all stash entries for `root`.
pub fn list(vcs: &VcsManager, root: &Path) -> TgResult<Vec<Stash>> {
    vcs.stash_list(root)
}
...
pub fn unshelve(vcs: &VcsManager, root: &Path, shelf: &Shelf) -> TgResult<()> {
